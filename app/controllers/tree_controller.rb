class TreeController < ApplicationController
  ...
  def show
    ...
    data_table.add_rows(Judge.where(country_tree: params[:id].upcase).asc(:name).collect{|j| [{
    	v: j.id.to_s,
    	f:"<div class='judge #{j.level_to_word}'><p>#{j.name_output}</p><img class='image-flag' src='/assets/flags/#{j.country}.png' title='#{IsoCountryCodes.find(j.country).name}' /><div class='level level-#{j.level_to_short}'>#{j.level_to_short}</div><img class='profile' src='http://apps.magicjudges.org/dci/avatar?dci=#{j.dci_number}&size=120' /></div>"}, j.parent_judge.try(:id).try(:to_s), "#{j.name} (#{j.level})"]})
